...
static DEFINE_SPINLOCK(shim_llist_lock);
...
static int shim_pr_dbg(const char *fmt, va_list args)
{
	char *mod_fmt = NULL;
	int ret = -1;
	size_t mod_fmt_len = 0;

	mod_fmt_len = strlen(fmt) + strlen(KERN_DEBUG) + 1;
	mod_fmt = kmalloc(mod_fmt_len, GFP_ATOMIC);
	...
	scnprintf(mod_fmt, mod_fmt_len, "%s%s", KERN_DEBUG, fmt);
...
static int shim_pr_info(const char *fmt, va_list args)
{
	...
	mod_fmt_len = strlen(fmt) + strlen(KERN_INFO) + 1;
	mod_fmt = kmalloc(mod_fmt_len, GFP_ATOMIC);
	...
	scnprintf(mod_fmt, mod_fmt_len, "%s%s", KERN_INFO, fmt);
...
static int shim_pr_err(const char *fmt, va_list args)
{
	...
	mod_fmt_len = strlen(fmt) + strlen(KERN_ERR) + 1;
	mod_fmt = kmalloc(mod_fmt_len, GFP_ATOMIC);
	...
	scnprintf(mod_fmt, mod_fmt_len, "%s%s", KERN_ERR, fmt);
...
static unsigned char shim_nbuf_get_chksum_done(void *nbuf)
{
	...
	/*
	 * CHECKSUM_PARTIAL means L3/L4 checksum is not complete yet and must be
	 * generated by offload logic. Only advertise checksum-done when the stack
	 * marks it as already complete.
	 */
...
static void *shim_llist_node_alloc(void)
{
	...
	if (!llist_node)
		...
		INIT_LIST_HEAD(&llist_node->head);
...
static void shim_llist_add_node_tail(void *llist, void *llist_node)
{
	...
	if (!shim_llist || !llist_node)
		return;

	spin_lock_irqsave(&shim_llist_lock, flags);

	list_add_tail(llist_node, &shim_llist->head);

	shim_llist->len += 1;

	spin_unlock_irqrestore(&shim_llist_lock, flags);
}
...
{
	struct shim_llist_node *node = NULL;
	struct shim_llist_node *iter = NULL;
	struct shim_llist *shim_llist = NULL;
	unsigned long flags = 0;
	bool found = false;
	...
	list_for_each_entry(iter, &shim_llist->head, head) {
		if (iter == node) {
			found = true;
			break;
		}
